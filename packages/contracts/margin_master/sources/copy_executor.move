...
const EUnauthorizedVault: u64 = 6;
...
public struct CopyRelation has copy, drop, store {
    trader: address,
    follower: address,
...
public fun execute_copy_trade<T>(
    ...
    // SECURITY FIX: Identify follower by the Vault owner, NOT the sender (which is the bot)
    ...
    // Open position for follower (delegated)
...
public entry fun execute_copy_trade_entry<T>(
    ...
    // Transfer position to vault owner (follower), NOT sender (bot)
    transfer::public_transfer(position, vault::owner(follower_vault));
}
...
public fun batch_execute_copy_trades<T>(
    ...
    while (i < relations_len && vault_idx < vaults_len) {
        ...
        if (relation.is_active) {
            ...
            // SECURITY CHECK: Ensure provided vault belongs to the expected follower
            ...
            let follower_quantity = if (
                relation.max_position_size > 0 && follower_quantity > relation.max_position_size
            ) {
            ...
            if (follower_quantity > 0) {
                let follower_margin =
                    (follower_quantity * entry_price) / ((leverage as u64) * 1000000);
                let follower_margin = if (follower_margin < 1000000) { 1000000 } else {
                    follower_margin
                };
...
public fun get_active_follower_count(registry: &CopyRelationRegistry, trader: address): u64 {
