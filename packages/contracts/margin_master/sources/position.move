...
public fun open_position<T>(
    ...
    assert!(
        position_type == POSITION_LONG || position_type == POSITION_SHORT,
        EInvalidPositionType,
    );
...
/// Open a position for a user (delegated version for copy trading)
/// Allows trusted packages (like copy_executor) to open positions on behalf of users
public(package) fun open_position_delegated<T>(
    ...
    // Deduct margin from vault (using delegated function)
    let margin = vault::deduct_margin_delegated(vault, margin_amount);

    // Create position
    let position_uid = object::new(ctx);
    let position_id = object::uid_to_inner(&position_uid);
    // OWNER IS VAULT OWNER, NOT SENDER
    let owner = vault::owner(vault);
    let timestamp = clock::timestamp_ms(clock);
...
public fun calculate_pnl<T>(position: &Position<T>, current_price: u64): (u64, bool) {
...
public fun get_liquidation_price<T>(position: &Position<T>): u64 {
    ...
    if (position.position_type == POSITION_LONG) {
        ...
        if (price_diff >= position.entry_price) 0 else position.entry_price - price_diff
    } else {
...
/// Set stop-loss price
public fun set_stop_loss<T>(position: &mut Position<T>, stop_loss_price: u64, ctx: &TxContext) {
    assert!(tx_context::sender(ctx) == position.owner, EUnauthorized);
    position.stop_loss_price = option::some(stop_loss_price);
}

/// Set take-profit price
public fun set_take_profit<T>(position: &mut Position<T>, take_profit_price: u64, ctx: &TxContext) {
    assert!(tx_context::sender(ctx) == position.owner, EUnauthorized);
    position.take_profit_price = option::some(take_profit_price);
}

/// Mark position as copy trade (called by copy executor)
public(package) fun mark_as_copy_trade<T>(position: &mut Position<T>, original_position_id: ID) {
